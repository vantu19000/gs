...
class HBActionEstateHouse extends hbaction{	
	...
	public function save(){
		...
		if($this->input->get('id')){				
			$result = $wpdb->update("{$wpdb->prefix}hbpro_estate_houses", $data, array('id'=>$this->input->get('id')));
			wp_safe_redirect(admin_url('admin.php?page=hb_estatehouse&layout=edit&id='.$this->input->get('id')));
		}else{
			$result = $wpdb->insert("{$wpdb->prefix}hbpro_estate_houses", $data);
			wp_safe_redirect(admin_url('admin.php?page=hb_estatehouse&layout=edit&id='.$wpdb->insert_id));
		}
		
		
		if($result){
			hb_enqueue_message(__('Adding success','hbpro'));
			$_SESSION['estatehouse']['data'] = false;
		}
		else{
			//neu that bai luu du lieu nay lai de user khong phai nhap lai du lieu nay
			$_SESSION['estatehouse']['data'] = (object)$data;
