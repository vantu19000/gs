...
class HBActionEstate extends hbaction{		
	
	
	public function get_table($table = '#__hbpro_estates',$primary_key = array('id')){
		return parent::get_table($table,$primary_key);
	}
	...
	public function save(){
		...
		debug($this->table->id);
		debug($post);
		debug($this->table->get_properties());die;
		
		$image = $this->input->get('image');
		
		wp_safe_redirect(admin_url('admin.php?page=hb_estate&layout=edit&id='.$id));
		if($result){
			hb_enqueue_message('Thêm Dự án thành công');
			$_SESSION['estate']['data'] = false;
		}
		else{
			//neu that bai luu du lieu nay lai de user khong phai nhap lai du lieu nay
			$_SESSION['estate']['data'] = (object)$data;
